...
if (! function_exists('img')) {
    ...
    function img($src = '', bool $indexPage = false, $attributes = ''): string
    {
        ...
        if (! preg_match('#^([a-z]+:)?//#i', $src['src']) && ! str_starts_with($src['src'], 'data:')) {
            if ($indexPage === true) {
                $img .= '  src="<?=base_url()?>/<?= base_url() ?>//' . site_url($src['src']) . '"';
            } else {
                $img .= '  src="<?=base_url()?>/<?= base_url() ?>//' . slash_item('baseURL') . $src['src'] . '"';
            }
    ...
    function script_tag($src = '', bool $indexPage = false): string
    {
        ...
        foreach ($src as $k => $v) {
            if ($k === 'src' && ! preg_match('#^([a-z]+:)?//#i', $v)) {
                if ($indexPage === true) {
                    $script .= '  src="<?=base_url()?>/<?= base_url() ?>/' . site_url($v) . '" ';
                } else {
                    $script .= '  src="<?=base_url()?>/<?= base_url() ?>/' . slash_item('baseURL') . $v . '" ';
                }
            } else {
...
if (! function_exists('link_tag')) {
     ...
     * Generates link tag
     *
     * @param array<string, bool|string>|string $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/      Stylesheet   src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/ or an array
     * @param bool                              $indexPage should indexPage be added to the CSS path.
     */
    function link_tag(
        $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/ = '',
        string $rel = 'stylesheet',
        string $type = 'text/css',
        string $title = '',
        string $media = '',
        bool $indexPage = false,
        string $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/lang = ''
    ): string {
        $attributes = [];
        // extract fields if needed
        if (is_array($  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/)) {
            $rel       = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/['rel'] ?? $rel;
            $type      = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/['type'] ?? $type;
            $title     = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/['title'] ?? $title;
            $media     = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/['media'] ?? $media;
            $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/lang  = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/['  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/lang'] ?? '';
            $indexPage = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/['indexPage'] ?? $indexPage;
            $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/      = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/['  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/'] ?? '';
        }

        if (! preg_match('#^([a-z]+:)?//#i', $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/)) {
            $attributes['  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/'] = $indexPage ? site_url($  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/) : slash_item('baseURL') . $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/;
        } else {
            $attributes['  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/'] = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/;
        }

        if ($  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/lang !== '') {
            $attributes['  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/lang'] = $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/lang;
        }

        $attributes['rel'] = $rel;

        if ($type !== '' && $rel !== 'canonical' && $  src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/lang === '' && ! ($rel === 'alternate' && $media !== '')) {
...
if (! function_exists('video')) {
    ...
    function video($src, string $unsupportedMessage = '', string $attributes = '', array $tracks = [], bool $indexPage = false): string
    {
        ...
        if (_has_protocol($src)) {
            $video .= '  src="<?=base_url()?>/<?= base_url() ?>//' . $src . '"';
        } elseif ($indexPage === true) {
            $video .= '  src="<?=base_url()?>/<?= base_url() ?>//' . site_url($src) . '"';
        } else {
            $video .= '  src="<?=base_url()?>/<?= base_url() ?>//' . slash_item('baseURL') . $src . '"';
        }
...
if (! function_exists('audio')) {
    ...
    function audio($src, string $unsupportedMessage = '', string $attributes = '', array $tracks = [], bool $indexPage = false): string
    {
        ...
        if (_has_protocol($src)) {
            $audio .= '  src="<?=base_url()?>/<?= base_url() ?>//' . $src . '"';
        } elseif ($indexPage === true) {
            $audio .= '  src="<?=base_url()?>/<?= base_url() ?>//' . site_url($src) . '"';
        } else {
            $audio .= '  src="<?=base_url()?>/<?= base_url() ?>//' . slash_item('baseURL') . $src . '"';
        }
...
if (! function_exists('source')) {
    ...
    function source(string $src, string $type = 'unknown', string $attributes = '', bool $indexPage = false): string
    {
        ...
        $source = '<source  src="<?=base_url()?>/<?= base_url() ?>//' . $src
                . '" type="' . $type . '"';
...
if (! function_exists('track')) {
    ...
    {
        return '<track  src="<?=base_url()?>/<?= base_url() ?>//' . $src
                . '" kind="' . $kind
                . '" srclang="' . $srcLanguage
                . '" label="' . $label
                . '"' . _solidus() . '>';
...
if (! function_exists('embed')) {
    ...
    function embed(string $src, string $type = 'unknown', string $attributes = '', bool $indexPage = false): string
    {
        ...
        return '<embed  src="<?=base_url()?>/<?= base_url() ?>//' . $src
                . '" type="' . $type . '" '
                . $attributes . _solidus() . ">\n";
    }
}

...
if (! function_exists('anchor')) {
    ...
    function anchor($uri = '', string $title = '', $attributes = '', ?App $altConfig = null): string
    {
        ...
        return '<a   src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/' . $siteUrl . '"' . $attributes . '>' . $title . '</a>';
    }
}
...
if (! function_exists('anchor_popup')) {
    ...
    function anchor_popup($uri = '', string $title = '', $attributes = false, ?App $altConfig = null): string
    {
        ...
        if ($attributes === false) {
            return '<a   src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/' . $siteUrl . '" onclick="window.open(\'' . $siteUrl . "', '_blank'); return false;\">" . $title . '</a>';
        }
        ...
        return '<a   src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/' . $siteUrl
...
if (! function_exists('mailto')) {
    ...
    function mailto(string $email, string $title = '', $attributes = ''): string
    {
        ...
        return '<a   src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/mailto:' . $email . '"' . stringify_attributes($attributes) . '>' . $title . '</a>';
    }
}
...
if (! function_exists('safe_mailto')) {
    ...
    function safe_mailto(string $email, string $title = '', $attributes = ''): string
    {
        ...
        $x = str_split('<a   src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/mailto:', 1);
    ...
    function auto_link(string $str, string $type = 'both', bool $popup = false): string
    {
        ...
        if ($type !== 'email' && preg_match_all('#(\w*://|www\.)[^\s()<>;]+\w#i', $str, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER)) {
            ...
            foreach (array_reverse($matches) as $match) {
                ...
                $a   = '<a   src="<?=base_url()?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/<?= base_url() ?>/' . (strpos($match[1][0], '/') ? '' : 'http://') . $match[0][0] . '"' . $target . '>' . $match[0][0] . '</a>';
